    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm max-w-3xl">
        <form action="{{ route('admin.artikel.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-1">
                ...
                <x-text-input id="title" name="title" type="text" required class="w-full mt-1 block" placeholder="Contoh: 5 Kunci Utama Lolos Beasiswa AAS ke Australia" :value="old('title')" />
            ...
            <div class="space-y-1">
                ...
                <x-text-input id="author" name="author" type="text" required class="w-full mt-1 block" placeholder="Contoh: Sarah Wijaya (AAS Awardee)" :value="old('author')" />
            ...
            <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1">
                        <label for="thumbnail_file" class="text-xs font-semibold text-slate-500 block">Unggah Gambar (JPG/PNG/WebP)</label>
                    ...
                    <div class="space-y-1">
                        <label for="thumbnail_url" class="text-xs font-semibold text-slate-500 block">Atau Gunakan URL Gambar Online</label>
                        <x-text-input id="thumbnail_url" name="thumbnail_url" type="url" class="w-full mt-1 block bg-white" placeholder="https://images.unsplash.com/..." :value="old('thumbnail_url')" />
            ...
            <div class="space-y-1">
                ...
                <textarea id="excerpt" name="excerpt" rows="2" class="w-full border-slate-200 focus:border-[#1E3A5F] focus:ring-[#1E3A5F] rounded-xl shadow-sm bg-slate-50 text-slate-800 text-sm py-2.5 px-4 mt-1" placeholder="Masukkan ringkasan singkat artikel yang muncul di daftar blog (maksimal 200 karakter).">{{ old('excerpt') }}</textarea>
            ...
            <div class="space-y-1">
                ...
                <textarea id="content" name="content" rows="12" required class="w-full border-slate-200 focus:border-[#1E3A5F] focus:ring-[#1E3A5F] rounded-xl shadow-sm bg-slate-50 text-slate-800 text-sm font-mono py-2.5 px-4 mt-1" placeholder="Tulis atau paste isi artikel Anda di sini...">{{ old('content') }}</textarea>
