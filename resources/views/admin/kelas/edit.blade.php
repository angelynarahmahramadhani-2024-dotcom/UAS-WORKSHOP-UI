<x-admin-layout>
    <x-slot name="title">Edit Kelas Premium</x-slot>
    <x-slot name="header">Edit Kelas Premium</x-slot>
    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm max-w-2xl">
        <form action="{{ route('admin.kelas.update', $class->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1">
                ...
                <x-text-input id="judul" name="judul" type="text" required class="w-full mt-1 block" placeholder="Contoh: Mastering IELTS Preparation Bootcamp" :value="old('judul', $class->judul)" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    ...
                    <x-text-input id="harga" name="harga" type="number" min="0" required class="w-full mt-1 block" placeholder="Contoh: 150000" :value="old('harga', number_format($class->harga, 0, '', ''))" />
                ...
                <div class="space-y-1">
                    ...
                    <x-text-input id="thumbnail" name="thumbnail" type="url" class="w-full mt-1 block" placeholder="https://unsplash.com/..." :value="old('thumbnail', $class->thumbnail)" />
            ...
            <div class="space-y-1">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3" required class="w-full border-slate-200 focus:border-[#1E3A5F] focus:ring-[#1E3A5F] rounded-xl shadow-sm bg-slate-50 text-slate-800 text-sm py-2.5 px-4 mt-1" placeholder="Masukkan ringkasan materi bimbingan.">{{ old('deskripsi', $class->deskripsi) }}</textarea>
            ...
            <div class="space-y-1">
                ...
                <textarea id="konten_publik" name="konten_publik" rows="3" class="w-full border-slate-200 focus:border-[#1E3A5F] focus:ring-[#1E3A5F] rounded-xl shadow-sm bg-slate-50 text-slate-800 text-sm py-2.5 px-4 mt-1" placeholder="Teaser promosi yang dapat dibaca oleh user yang belum berlangganan.">{{ old('konten_publik', $class->konten_publik) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    ...
                    <x-text-input id="link_zoom" name="link_zoom" type="url" class="w-full mt-1 block" placeholder="https://zoom.us/j/..." :value="old('link_zoom', $class->link_zoom)" />
                ...
                <div class="space-y-1">
                    ...
                    <x-text-input id="link_rekaman" name="link_rekaman" type="url" class="w-full mt-1 block" placeholder="https://youtube.com/embed/..." :value="old('link_rekaman', $class->link_rekaman)" />
            ...
            <div class="space-y-1">
                ...
                @if($class->file_materi)
                    <p class="text-xs text-slate-400 font-medium mb-1">File saat ini: 
                        <a href="{{ asset($class->file_materi) }}" target="_blank" class="text-[#1E3A5F] hover:underline font-bold">{{ basename($class->file_materi) }}</a>
                    </p>
                @endif
  ...
  {"judul": "Sesi 2: Writing Essay", "materi": "Menulis draf awal"}
]'>{{ old('kurikulum', $class->kurikulum ? json_encode($class->kurikulum, JSON_PRETTY_PRINT) : '') }}</textarea>
            ...
            <div class="space-y-1 max-w-xs">
                ...
                <select name="status" id="status" required class="w-full border-slate-200 focus:border-[#1E3A5F] focus:ring-[#1E3A5F] rounded-xl shadow-sm bg-slate-50 text-slate-800 text-sm py-2.5 px-4 mt-1">
                    <option value="aktif" {{ old('status', $class->status) == 'aktif' ? 'selected' : '' }}>Aktif (Tampilkan)</option>
                    <option value="nonaktif" {{ old('status', $class->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif (Sembunyikan)</option>
                </select>
